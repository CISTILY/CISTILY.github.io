...
export function generateStaticParams() {
  return collections.flatMap((collection) =>
    getEntries(collection).map((entry) => ({ collection, slug: entry.slug })),
  );
}
...
export default async function DetailPage({ params }: Props) {
  ...
  return (
    <article className="detail-content">
      <a className="back-link" href={`/${entry.collection}/`}>
        <ArrowLeft size={16} /> Tất cả {collectionTitle}
      </a>
      <header className="page-heading detail-heading">
        <span className="page-kicker">{collectionTitle.toUpperCase()}</span>
        <h1>{entry.title}</h1>
        <p>{entry.summary}</p>
        <div className="entry-meta">
          ...
          {entry.images.length > 0 && <span>{entry.images.length} ảnh</span>}
        </div>
      </header>
      {showCover && (
          ...
          {entry.images.map((image, index) => (
            <figure key={`${image.src}-${index}`}>
              <a
                ...
                target="_blank"
                rel="noreferrer"
                aria-label={`Mở ảnh đầy đủ: ${image.alt}`}
              >
      ...
      <footer className="detail-footer">
        ...
        <a className="back-link" href={`/${entry.collection}/`}>
          <ArrowLeft size={16} /> Quay lại {collectionTitle}
        </a>
      </footer>
    </article>
  );
