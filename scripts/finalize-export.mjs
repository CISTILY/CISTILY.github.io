...
const routes = manifest.routes
  .filter((route) => route.status === 'rendered')
...
const withinOutput = (name) => {
  ...
  if (resolved !== output && !resolved.startsWith(output + path.sep))
    throw new Error('Invalid export path: ' + name);
  return resolved;
};
for (const route of routes) {
  ...
  fs.copyFileSync(
    withinOutput(relative + '.html'),
    path.join(directory, 'index.html'),
  );
  if (fs.existsSync(withinOutput(relative + '.rsc')))
    fs.copyFileSync(
      withinOutput(relative + '.rsc'),
      path.join(directory, 'index.rsc'),
    );
}
...
for (const route of routes) {
  ...
  for (const match of html.matchAll(/\b(?:href|src)="([^"]+)"/g)) {
    ...
    const url = new URL(
      href,
      'https://local.invalid' + (route.endsWith('/') ? route : route + '/'),
    );
    ...
    if (!exists && !fs.existsSync(file + '.html'))
      throw new Error(
        `[content] ${route}: không tìm thấy tài nguyên/đường dẫn ${href}. Kiểm tra file trong public hoặc đường dẫn Markdown.`,
      );
  }
}
console.log(
  `Static export ready: ${routes.length} pages. Directory URLs and local links/assets verified.`,
);
