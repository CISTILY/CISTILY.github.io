...
function asset(value: string, fail: (message: string) => never) {
  if (!value) return '';
  if (!safeLink(value) || value.startsWith('mailto:') || value.includes('..'))
    fail('Ảnh phải dùng /images/ten-anh.jpg hoặc URL https:// hợp lệ.');
  return value;
}
export function parseEntry(
  ...
  slug: string,
  collection: Collection,
  source = `${collection}/${slug}.md`,
): Entry | null {
  const fail = (message: string): never => {
    throw new Error(`[content] ${source}: ${message}`);
  };
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug))
    fail('Tên file chỉ gồm chữ thường không dấu, số và dấu gạch ngang.');
  const match = text
    .replace(/^\uFEFF/, '')
    .match(/^---\r?\n([\s\S]*?)\r?\n---(?:\r?\n|$)([\s\S]*)$/);
  ...
  if (!data || typeof data !== 'object' || Array.isArray(data))
    fail('Phần thông tin phải là danh sách tên trường: giá trị.');
  const meta = data as Record<string, unknown>;
  if (meta.published !== undefined && typeof meta.published !== 'boolean')
    fail('published phải là true hoặc false, không đặt trong dấu nháy.');
  if (meta.published !== true) return null;
  const string = (key: string, required = false) => {
    const value = meta[key];
    if (value === undefined && !required) return '';
    if (typeof value !== 'string' || (required && !value.trim()))
      fail(`${key} phải là chuỗi${required ? ' không rỗng' : ''}.`);
  ...
  if (
    ...
    fail('date phải là ngày thực theo dạng "YYYY-MM-DD".');
  if (
    meta.order !== undefined &&
    (typeof meta.order !== 'number' || !Number.isFinite(meta.order))
  )
    fail('order phải là một số.');
  if (meta.sample !== undefined && typeof meta.sample !== 'boolean')
    fail('sample phải là true hoặc false.');
  if (
    meta.tags !== undefined &&
    (!Array.isArray(meta.tags) ||
      meta.tags.some((tag) => typeof tag !== 'string'))
  )
    fail('tags phải là danh sách chuỗi.');
  if (meta.images !== undefined && !Array.isArray(meta.images))
    fail('images phải là danh sách ảnh.');
  const images: EntryImage[] = ((meta.images ?? []) as unknown[]).map(
    (image) => {
      ...
      if (
        ...
        fail('Mỗi ảnh cần src và alt không rỗng.');
      if (item.caption !== undefined && typeof item.caption !== 'string')
        fail('caption phải là chuỗi.');
      return {
        src: asset(item.src as string, fail),
  ...
  return {
    ...
    cover,
    coverAlt: string('coverAlt') || images[0]?.alt || title,
    sample: meta.sample === true,
    images,
    html: renderMarkdown(body || summary),
    minutes: Math.max(
      1,
      Math.ceil(body.split(/\s+/u).filter(Boolean).length / 220),
    ),
  };
}
...
export function getEntries(
  ...
  return fs
    ...
    .filter(
      (file) =>
        file.isFile() && file.name.endsWith('.md') && !/^[_\.]/.test(file.name),
    )
    .map((file) =>
      parseEntry(
